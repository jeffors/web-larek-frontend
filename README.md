...
- `_catalog: Product[]` - массив всех товаров магазина
- `_order: IOrder` - данные для оформления заказа
- `formErrors: FormErrors` - хранит различные ошибки формы

Класс предоставляет слелующий набор методов:
- `get catalog(): Product[]` - возвращает каталог товаров
- `set catalog(items: IProduct[])` - устанавливает каталог товаров интернет-магазина
- `get basket()` - возвращает корзину товара
